...
pair<int, int> get_max(int l, int r, vector<vector<pair<int, int>>>& sparse) {
    int sz = log2n[(r - l) + 1];
...
void solve() {
    ...
    vector<vector<pair<int, int>>> sparse(power + 1, vector<pair<int, int>>(n, {0, 0}));
    for (int i = 0; i < n; i++) {
        sparse[0][i] = { arr[i], i };
    }
    ...
    for (int test = 0; test < q; test++) {
        int l, r;
        cin >> l >> r;
        cout << get_max(l, r, sparse).second + 1 << ' ';
    }
}
